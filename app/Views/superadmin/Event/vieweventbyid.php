        ...
        <div class="row">
            <?php foreach ($tb_event as $event) :
                if ($event['k_check_u_id'] != NULL) {
                    if ($event['k_id'] == $tb_event_k_id['k_id']) { ?>
                        <div class="col-12 col-md-4 col-lg-4">
                            <article class="article article-style-c">
                                <div class="article-header">
                                    <div class="article-image" data-background="<?= base_url('Event/' . $tb_event_k_id['k_foto']) ?>">
                                    </div>
                                </div>
                                <div class="article-details">
                                    <?php $current_time = time();
                                    $waktu = $current_time - strtotime($tb_event_k_id['k_create_at']);
                                    $hari = floor($waktu / (3600 * 24));
                                    $jam = floor(($waktu % (3600 * 24)) / 3600);
                                    $menit = floor(($waktu % 3600) / 60);
                                    ?>
                                    <div class="article-category"><a href="#"><?= $tb_event_k_id['k_jeniskegiatan'] ?></a>
                                        <div class="bullet"></div> <a href="#"><?= $hari . " hari, " . $jam . " jam, " . $menit . " menit yang lalu" ?></a>
                                    </div>
                                    <div class="article-title">
                                        <h2><a class="text-blue"><?= $tb_event_k_id['k_nama'] ?></a></h2>
                                    </div>
                                    <p><?= $tb_event_k_id['k_deskripsikegiatan'] ?></p>
                                    <?php
                                    foreach ($tb_user as $data) {
                                        if ($tb_event_k_id['u_id'] == $data['u_id']) {
                                            $u_id = $data['u_id'];
                                    ?>
